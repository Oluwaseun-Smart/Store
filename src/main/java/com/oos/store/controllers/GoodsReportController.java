...
public class GoodsReportController implements Initializable {
    ...
    private void writeToPDF() {
        ...
        try {
            ...
            document.add(new Paragraph("EL & BEE TECHNOLOGIES", font));
            document.add(new Paragraph(new Date() + ""));
            document.add(new Paragraph("----------------------------------------------------------------------------------------------------------------------------------"));

            //Table
            PdfPTable salesTableReport = new PdfPTable(9);

            PdfPCell cell = new PdfPCell(new Paragraph("Sales Informations"));
            cell.setColspan(18);
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setBackgroundColor(BaseColor.CYAN);

            salesTableReport.addCell(cell);

            salesTableReport.addCell("Gadget");
            salesTableReport.addCell("Model");
            salesTableReport.addCell("Price");
            salesTableReport.addCell("PriceSold");
            salesTableReport.addCell("Profit");
            salesTableReport.addCell("Loss");
            salesTableReport.addCell("CusName");
            salesTableReport.addCell("SellName");
            salesTableReport.addCell("D&T");

            for (Sales col : allSales) {
                salesTableReport.addCell(col.getGadgetName());
                salesTableReport.addCell(col.getModel());
                salesTableReport.addCell(df.format(col.getPrice()));
                salesTableReport.addCell(df.format(col.getPriceSold()));
                salesTableReport.addCell(df.format(col.getProfit()));
                salesTableReport.addCell(df.format(col.getLoss()));
                salesTableReport.addCell(col.getCustomerName());
                salesTableReport.addCell(col.getSeller());
                salesTableReport.addCell(col.getDateOfSells());
            }

            document.add(salesTableReport);
            ...
            alert.setHeaderText("Document Generated Successfully");
